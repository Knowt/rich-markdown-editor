...
const List = styled.ul`
  ...
  &.not-active {
    opacity: 0;
    visibility: hidden;
    transform: translate(4px) scale(0.95);
  }
...
const calcPopoutStyles = (input: CalcPopoutStylesInput) => {
  ...
  if (refRect && popoutRefRect) {
    ...
    let top =
      refRect.top - refRect.height / 2 + window.scrollY + POPOUT_PADDING;
    ...
    if (position === "left") {
      const left =
        refRect.x - refRect.width / 2 - popoutRefRect.width + POPOUT_PADDING;
...
const ToolbarPopout = forwardRef<HTMLElement, Props>(
  (
    { id, position, items, close, isActive, commands, theme, shouldLightIcon },
    ref
  ) => {
    ...
    useEffect(() => {
      if (typeof ref !== "function" && ref?.current && isActive) {
        setPopoutStyles(
          calcPopoutStyles({
            ref,
            popoutRef,
          ...
          {items.map((item, index) => {
            ...
            return (
              ...
              >
                {Icon ? (
                  <Icon
                    className={getIconClasses(shouldLightIcon)}
                    color={
                      item.name === "highlight_yellow "
                        ? CUSTOM_ICON_YELLOW
                        : iconColor || theme.toolbarItem
                    }
                    size={22}
                    {...iconSVGProps}
                  />
