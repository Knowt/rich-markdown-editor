    ...
    commands({ schema }) {
        return {
            createTable: ({ rowsCount, colsCount }) => (state, dispatch) => {
                ...
                const nodes = prosemirror_utils_knowt_1.createTable(schema, rowsCount, colsCount);
            ...
            setColumnAttr: ({ index, alignment }) => (state, dispatch) => {
                const cells = prosemirror_utils_knowt_1.getCellsInColumn(index)(state.selection) || [];
            ...
            addRowAfter: ({ index }) => (state, dispatch) => {
                if (index === 0) {
                    const tr = prosemirror_utils_knowt_1.addRowAt(index + 2, true)(state.tr);
                    dispatch(prosemirror_utils_knowt_1.moveRow(index + 2, index + 1)(tr));
                }
                else {
                    dispatch(prosemirror_utils_knowt_1.addRowAt(index + 1, true)(state.tr));
                }
            },
        ...
        return {
            ...
            Enter: (state, dispatch) => {
                if (!prosemirror_tables_contently_1.isInTable(state))
                    return false;
                const cells = prosemirror_utils_knowt_1.getCellsInColumn(0)(state.selection) || [];
                dispatch(prosemirror_utils_knowt_1.addRowAt(cells.length, true)(state.tr));
                return true;
            },
        };
    }
